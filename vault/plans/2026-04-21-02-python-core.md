...
def test_top_gets_upper_portion():
    ...
    # Top's bottom edge sits above bottom's top edge — no vertical overlap.
    _x, top_bottom = apply_affine_to_point(top_out.transform, (0.0, 1024.0))
    _x, bot_top = apply_affine_to_point(bot_out.transform, (0.0, 0.0))
    assert top_bottom < bot_top, f"top_bottom={top_bottom} should be < bot_top={bot_top}"
    # Both boxes actually fit inside the glyph bbox.
    assert 0.0 <= top_bottom < 1024.0
    assert 0.0 < bot_top <= 1024.0
    kinds = [c.__class__.__name__ for c in constraints]
